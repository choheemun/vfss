...
def root_mean_squared_error(y_true, y_pred):
    return tf.sqrt(tf.reduce_mean(tf.keras.losses.mean_squared_error(y_true, y_pred)))
...
def train():
    ...
    for _ in range(2):      # (None, 56, 56, 256)-> (None, 7,7,512)
        for _ in range(3):
    ...
    train_step = tf.train.GradientDescentOptimizer(learning_rate=1.).minimize(loss, global_step=global_step)
