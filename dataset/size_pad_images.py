...
for image in images:
    ...
    w = width
    h = round(w / ratio)   # 아무튼 결과로 sys.arg[2] 인 height와 유사한 값이 나온다
    if h > height:
        h = height
        w = round(h * ratio)
    ...
    scaled_img = cv2.resize(img, (w, h), interpolation=interp) # ration 를 유지하면서
    padded_img = cv2.copyMakeBorder(
        scaled_img,0,pad_bottom,0,pad_right,borderType=cv2.BORDER_CONSTANT,value=[0,0,0])  # 나머지는 padding을 준다
