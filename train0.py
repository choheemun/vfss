...
def get_dataset(dataset_dir, is_training=False, file_pattern=None, reader=None):  #dataset_dir='./dataset/photo/'
    """"""
    tfrecords = glob.glob(dataset_dir + '/records/' + 'coco_tfrecord')    #따라서 "convert_coco.py"로 만든 tfrecord를 미리 'datasets/photo/record'에 옮겨두어야 한다.
    image, ih, iw, gt_boxes, gt_masks, num_instances, img_id = read(tfrecords)

    image, gt_boxes, gt_masks = preprocess_image(image, gt_boxes, gt_masks, is_training)  # is_training=True (def train 참조)
...
def train():
    ...
    gt_mask=tf.expand_dims(gt_mask,axis=3)          # network output 은 1x1 conv 를 반복하여 depth=1이 된다. 하지만 depth가 없는 것이 아니다.
                                                    # 따라서 gt_mask에 3rd dim을 추가해주어야 한다.
    ...
    with tf.Session() as sess:
        ...
        ckpt = tf.train.get_checkpoint_state('dataset/checkpoint/checkpoint0')
        ...
        for step in range(10000):
            ...
            if step % 5 == 0:
                ...
                checkpoint_path = os.path.join('dataset/checkpoint/checkpoint0', 'nn.ckpt')
